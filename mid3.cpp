...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1)
        tol = atof(argv[1]);
    cout << "unit test running..." << endl << endl;
    unit_test(tol);
...
int unit_test(double tol)
{
    ...
    int ret=0;
    /*    float data[ROWS][COLS] = {{135.000000, 50.000000, 13.572088},
    ...
    for (int i=0; i<ROWS; ++i) {
        pond = getPonderal(data[i][0], data[i][1]);
        cout << setw(10) << left << fixed << setprecision(6);
        if ((pond <= (data[i][2]+tol)) && (pond >= (data[i][2]-tol))) {
            cout << setw(10) << left << fixed << data[i][0] << "  " << data[i][1] << "  " << data[i][2]
                << "  " << getPonderal(data[i][0], data[i][1]) << endl;
        }
        else {
            cout << setw(10) << left << fixed << data[i][0] << "  " << data[i][1] << "  " << data[i][2]
                << "  " << getPonderal(data[i][0], data[i][1]) <<  "  <----- outside of tolerance" << endl;
            error++;
        }


    }
    cout << error << " errors found. " << endl;
    return ret;
}
#endif
